...
| `data-div-id` | `comparos-video-placement` | ID of the `<div>` the player mounts into. If a div with this id already exists in the page (publisher-placed placeholder), the engine mounts into it and only appends the `<video>` child if missing — it does **not** require the div to be pre-populated. If absent, the engine auto-creates the div next to the `<script>` tag (or in `<body>` if the tag lives in `<head>` — see gotcha 7) |
| `data-cache` | Prebid cache URL | Prebid cache endpoint |
| `data-config-url` | none | Base URL for **dynamic ("integrate-once") config**. When present alongside `data-placement-id`, the engine fetches `<config-url>/<placement-id>` at boot and applies the returned `RuntimeConfig` JSON over every attribute default above (adTag/adTags, video, bidders, floors, bias, divId, prebidUrl, sticky, refresh, etc.) — so demand and behaviour are backend-controlled with no tag edit. `resolveConfig()` never rejects: on fetch failure/timeout (1.5s) it falls back to the last-known-good copy cached in `localStorage`, then to the attribute defaults. Absent → fully static tag, unaffected |
| `data-placement-id` | none | Placement key used both for `data-config-url` lookups and as the telemetry dimension (see `data-account`/`data-beacon-url`) |
| `data-account` / `data-beacon-url` / `data-sample-rate` | none / none / `1` | When `data-beacon-url` + `data-account` + `data-placement-id` are all present, the engine fire-and-forgets lifecycle events (load, view, bid request/response, win, impression, complete, error) to the collector, sampled at `data-sample-rate`. Any one missing → telemetry is a no-op |
...
7. **`ensureMount()` and the "publisher-placed div" pattern (fixed on `dev`, unreleased as of 2026-09-03).** Ad-ops teams often need the `<script>` tag itself in `<head>` to satisfy their trafficking system, and separately mark the display spot with a plain `<div id="...">`. This exposed two real bugs, both fixed in `engine/player.js`:
   ...
   - Also added: a dedup guard (keyed on `placementId` or `divId`) so a duplicate tag pasted twice on one page (once for "execution", once "for placement" — a real thing a publisher did) no-ops instead of double-running the auction/IMA/beacons.
   - Root-cause incident: kurzy.cz's colleague copied **rendered** DOM out of DevTools (`data-atp-sticky="1"` + the sticky close button — both generated by `setupSticky()` at runtime, never authored) and pasted it as the placeholder, instead of a plain empty div.
   - Not yet merged to `main`/released — publishers on `@dev` (staging pattern, see [[engine-delivery-divergence]]) pick this up on push via `purge-dev-engine.yml`; pinned/loader-based prod tags need an actual version release to get it.
